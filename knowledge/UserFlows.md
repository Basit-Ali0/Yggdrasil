# User Flow Stories: PolicyGuard AI
...
**Related Docs:**
- [UserStories-PolicyGuard-AI.md](./UserStories-PolicyGuard-AI.md) - All user stories with acceptance criteria
...
These detailed user flow stories map the complete journey through PolicyGuard AI for two distinct user personas:
...
| 6.3 | Download completes | File: PolicyGuard_Report_DataFlow_2026-02-21.pdf | "Ready to share" |
...
- [UserStories-PolicyGuard-AI.md](./UserStories-PolicyGuard-AI.md) - All user stories
...
> [!IMPORTANT]
> **Why this matters:** Transparent Mapping — the user sees and approves every AI decision before any data is processed. This is what separates PolicyGuard from black-box tools and builds trust with compliance auditors.
...
> [!NOTE]
> **Conclusion:** Human-in-the-loop review is what separates PolicyGuard from black-box compliance tools.
...
Bottom action bar:
"Add Review Note" text input (ghost style). "Mark as False Positive" secondary button — removes the violation from the score calculation. "Confirm Violation" primary button — locks it as confirmed. Compliance score on the dashboard recalculates immediately after either action.
User confirms or overrides → backend: PATCH /api/violations/:id — updates status, recalculates score. Human-in-the-loop review is what separates PolicyGuard from black-box compliance tools.